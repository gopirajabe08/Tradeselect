...
export type CostInput = {
  ...
  qty: number;
  price: number;
  /** Distinguishes equity from F&O for STT/charges. Default = equity. */
  segment?: "EQUITY" | "FUT" | "OPT";
...
/** Securities Transaction Tax — varies by product + side + segment. */
...
/** NSE exchange transaction charge — varies by segment. */
function exchangeFor(input: CostInput, notional: number): number {
  const seg = input.segment ?? "EQUITY";
  if (seg === "EQUITY") return notional * 0.0000297;     // 0.00297 %
  if (seg === "FUT")    return notional * 0.0000173;     // 0.00173 %
  if (seg === "OPT")    return notional * 0.000495;      // 0.0495 %
  return 0;
}
...
/** Maps a Fyers-format symbol + product to a rough segment for cost lookup. */
export function inferSegment(symbol: string, productType: CostInput["productType"]): "EQUITY" | "FUT" | "OPT" {
  const u = symbol.toUpperCase();
  if (u.endsWith("CE") || u.endsWith("PE")) return "OPT";
  if (u.includes("FUT")) return "FUT";
  // Anything in NFO / MCX without explicit FUT/CE/PE — default to FUT
  if (u.startsWith("NFO:") || u.startsWith("MCX:") || u.startsWith("BFO:") || u.startsWith("CDS:")) {
    return "FUT";
  }
  return "EQUITY";
}
