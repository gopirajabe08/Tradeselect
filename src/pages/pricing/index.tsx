...
import { getWalletPricing } from '@/services/api';
...
export default function Page() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      try { const d = await getWalletPricing(); if (mounted) setData(d as Record<string, unknown>); }
  ...
  return (
    <Card>
      <Title level={3} style={{ margin: 0, marginBottom: 16, color: '#213654' }}>Pricing</Title>
      {loading && <Spin />}
