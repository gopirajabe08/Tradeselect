...
import { getPhoenixTemplatesFilters } from '@/services/api';
...
export default function Page() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      try { const f = await getPhoenixTemplatesFilters(); if (mounted) setFilters(f); }
      catch { /* fall through */ }
    ...
    <div>
      <Card style={{ marginBottom: 16 }}>
        <Row gutter={16} align="middle">
          <Col flex="auto">
            <Title level={3} style={{ margin: 0, color: '#213654' }}>Select Template</Title>
            <Text type="secondary">Pick a starting template</Text>
          </Col>
          <Col>
